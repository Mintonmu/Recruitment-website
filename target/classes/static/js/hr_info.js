 ...
 */
function infoUpdate() {
    let hrMobile = $("#tel").val();
    let hrPassword = $("#password").val();
    let hrName = $("#inputName").val();
    let hrEmail = $("#email").val();
    let data = {"hrMobile": hrMobile, "hrPassword": hrPassword, "hrName": hrName, "hrEmail": hrEmail};
    console.log(data);
    $.ajax({
        type: "post",
        url: "/hr/info/update",
...
function posJump(Posid) {
    var id = Posid
    window.location.href = "/position/" + id;
}

function posRemove(Posid) {
    var id = Posid
    console.log(id)
    $.ajax({
        type: "POST",
        url: "/hr/position/delete/" + id,
        dataType: "text",
        success: function (msg) {
            console.log(msg);
            if (msg === 1) {
